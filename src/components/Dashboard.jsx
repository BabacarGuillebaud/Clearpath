...
function ComingSoon({ title }) {
  return (
    <div style={{ background: '#fff', border: `1px solid ${border}`, borderRadius: '16px', padding: '40px', textAlign: 'center', color: muted, fontFamily: sans }}>
      <div style={{ fontSize: '14px', marginBottom: '6px', fontWeight: '500' }}>{title}</div>
      <div style={{ fontSize: '12px', color: hint }}>Coming soon</div>
    </div>
  )
}
...
function Dashboard({ user, onLogout }) {
  ...
  const metrics = user.initials === 'DO' ? dorothyMetrics : babacarMetrics
    ...
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: sans, background: bg }}>
      ...
      <aside style={{ width: '220px', background: '#fff', borderRight: `1px solid ${border}`, display: 'flex', flexDirection: 'column', minHeight: '100vh', position: 'sticky', top: 0, height: '100vh' }}>
        ...
        <div style={{ margin: '12px 12px 0', padding: '10px 12px', background: bg, borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: user.avatarBg, color: user.avatarColor, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: '500' }}>
            {user.initials}
          </div>
