...
const PROFILES = [
  { email: 'babacar@clearpath.app', name: 'Babacar', role: 'Budget France · EUR', flag: '🇫🇷', currency: 'EUR', initials: 'BA', avatarBg: '#dbeafe', avatarColor: '#1e40af' },
  { email: 'dorothy@clearpath.app', name: 'Dorothy', role: 'Fiscalité Australie · AUD', flag: '🇦🇺', currency: 'AUD', initials: 'DO', avatarBg: '#fce7f3', avatarColor: '#9d174d' },
]
...
function Login({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff', fontFamily: sansFont }}>
      <div style={{ width: '100%', maxWidth: '380px', padding: '2rem 1.5rem' }}>
        ...
        <p style={{ fontSize: '14px', color: textMuted, marginBottom: '2rem' }}>Choisissez votre profil pour continuer</p>
          ...
          {PROFILES.map(profile => (
            <button key={profile.email} onClick={() => setSelected(profile.email)}
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '12px', color: textMuted }}>{profile.role}</div>
              </div>
        ...
        <div style={{ marginBottom: '16px' }}>
          <label style={{ fontSize: '12px', color: textMuted, display: 'block', marginBottom: '6px' }}>Mot de passe</label>
          <input
            type="password"
            value={password}
            ...
            placeholder="••••••••"
            style={{ width: '100%', padding: '10px 12px', border: `1px solid ${border}`, borderRadius: '8px', fontSize: '14px', fontFamily: sansFont, outline: 'none' }}
          />
        </div>
        ...
        <button onClick={handleLogin} disabled={loading}
          style={{ width: '100%', padding: '12px', background: green, color: '#fff', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '500', cursor: 'pointer', fontFamily: sansFont, opacity: loading ? 0.7 : 1 }}>
